<script lang="ts">
	import { Music, Chat, Microphone, Upload, Webcam, ImagePaste } from "@gradio/icons";

	type source_types = "audio" | "speech" | "upload" | "microphone" | "webcam" | "clipboard" | null;
...
{#if unique_sources.length > 1}
	<span class="source-selection" data-testid="source-select">
		...
		{#if sources.includes("audio")}
			<button
				class="icon"
				class:selected={(active_source === "audio" || !active_source) && value !== null}
				aria-label="Audio"
				on:click={() => handle_select_source("audio")}><Music /></button
			>
		{/if}
