...
class extendedimage(StreamingInput, Component):
    """
    ...
    def __init__(
        ...
        *,
        ...
        """
        Parameters:
            value: A PIL extendedimage, numpy array, path or URL for the default value that extendedimage component is going to take. If a function is provided, the function will be called each time the app loads to set the initial value of this component.
            format: File format (e.g. "png" or "gif"). Used to save image if it does not already have a valid format (e.g. if the image is being returned to the frontend as a numpy array or PIL extendedimage). The format should be supported by the PIL library. Applies both when this component is used as an input or output. This parameter has no effect on SVG files.
        ...
        if streaming and self.sources != ["webcam"]:
            raise ValueError(
                "extendedimage streaming only available if sources is ['webcam']. Streaming not supported with multiple sources."
            )
